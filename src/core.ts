 ...
 */
export function signDataToJsonString(signData: unknown): string {
...
export function createVirtualPayUserCancelError(errMsg?: string): UserCancelError {
  const e = new Error(errMsg ?? '用户取消支付') as UserCancelError
  e.errCode = VIRTUAL_PAY_ERR_USER_CANCEL
  return e
}

export function toFailure(e: unknown): VirtualPaymentFailure {
...
export function rejectFlow(failure: VirtualPaymentFailure): VirtualPaymentFlowRejectError {
